...
export default function ProductCard({ product }) {
  ...
  // Safe image URL with error handling
  const getSafeImageUrl = (url) => {
    if (!url || url.includes('undefined') || url.includes('qwsqsq')) {
      return "https://via.placeholder.com/300x200/4A5568/FFFFFF?text=No+Image";
    }
    return url;
  };
    ...
    >
      <div className="relative">
        <img
          ...
          className="w-full h-48 object-cover"
          onError={(e) => {
            e.target.src = "https://via.placeholder.com/300x200/4A5568/FFFFFF?text=No+Image";
          }}
        />
        <div className="absolute top-3 left-3">
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
